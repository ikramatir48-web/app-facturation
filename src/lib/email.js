...
async function sendEmail(to, subject, html) {
  try {
    // Récupérer le token avec un timeout pour éviter EarlyDrop
    let accessToken = ''
    try {
      const { data } = await supabase.auth.getSession()
      accessToken = data?.session?.access_token || ''
    } catch(e) {
      console.warn('getSession failed, sending without token')
    }
  ...
  await sendEmail(
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;border-radius:8px;">
      ...
      <p style="color:#444;line-height:1.6;">
        ...
        Votre facture <strong>${numeroFacture}</strong> a été générée.
      </p>
...
export async function emailEnvoiDocument(to, nom, typeDoc, numeroDoc, numeroCommande) {
  const titres = {
    'BC': 'Bon de Commande',
    'BL': 'Bon de Livraison',
  }
  const titre = titres[typeDoc] || typeDoc

  await sendEmail(
    to,
    `${titre} ${numeroDoc} — Ijtihad Gaz`,
    `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;border-radius:8px;">
      <h1 style="color:#e85d04;font-size:28px;margin-bottom:4px;">Ijtihad Gaz</h1>
      <p style="color:#666;margin-bottom:32px;">Document disponible</p>
      
      <h2 style="color:#111;">Bonjour ${nom},</h2>
      <p style="color:#444;line-height:1.6;">
        Votre <strong>${titre}</strong> numéro <strong>${numeroDoc}</strong>
        relatif à la commande <strong>${numeroCommande}</strong> est disponible dans votre espace client.
      </p>
      
      <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:20px;margin:24px 0;text-align:center;">
        <div style="font-size:13px;color:#999;margin-bottom:8px;">Document</div>
        <div style="font-size:22px;font-weight:700;color:#e85d04;">${numeroDoc}</div>
        <div style="font-size:13px;color:#666;margin-top:4px;">${titre}</div>
      </div>
      
      <p style="color:#444;line-height:1.6;">
        Connectez-vous à votre espace client pour consulter et télécharger ce document.
      </p>
