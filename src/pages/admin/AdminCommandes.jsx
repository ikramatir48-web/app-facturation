...
export default function AdminCommandes() {
  ...
  async function load() {
    const { data } = await supabase
      .from('commandes')
      .select('*, profiles(id, nom, email, numero_client, telephone, adresse, condition_paiement, condition_paiement_autre, ice), livreurs(id, nom, prenom, immatriculation)')
  ...
  async function openDetail(cmd) {
    ...
    // Charger adresse de livraison
    let adresse = null
    const { data: cmdFull } = await supabase.from('commandes').select('adresse_livraison_id').eq('id', cmd.id).maybeSingle()
    if (cmdFull?.adresse_livraison_id) {
      const { data: adr } = await supabase.from('adresses_livraison').select('*').eq('id', cmdFull.adresse_livraison_id).maybeSingle()
      adresse = adr
    }
    setSelected({ cmd, lignes: lignesRes.data || [], bl: blRes.data, facture, adresse })
  }
  ...
  async function validerCommande(cmd) {
    ...
    try {
      ...
      try {
        ...
        let clientEmail = cmd.profiles?.email
        let clientNom = cmd.profiles?.nom
        if (!clientEmail) {
          const { data: p } = await supabase.from('profiles').select('email, nom').eq('id', cmd.client_id).maybeSingle()
          clientEmail = p?.email
          clientNom = p?.nom
        }
        if (clientEmail) {
  ...
  async function marquerPaye(cmd) {
    ...
    try {
      ...
      } catch(e) { console.error('Email paiement error:', e) }
      await load()
  ...
  function partagerAdresse(adresse) {
    if (!adresse) return
    const txt = `📍 ${adresse.label}\n${adresse.adresse}${adresse.ville ? ', ' + adresse.ville : ''}${adresse.latitude ? '\nhttps://maps.google.com/?q=' + adresse.latitude + ',' + adresse.longitude : ''}`
    if (navigator.share) {
      navigator.share({ title: 'Adresse de livraison', text: txt })
    } else {
  ...
  async function showPrintBL(bl, cmd, lignes) {
    ...
    const { data: cmdData } = await supabase.from('commandes').select('livreur_id, adresse_livraison_id').eq('id', cmd.id).maybeSingle()
    ...
    // Charger adresse de livraison
    let adresse = null
    const adresseId = cmdData?.adresse_livraison_id
    if (adresseId) {
    ...
    setPrintBL({ bl, commande: cmd, lignes: finalLignes, client: client || cmd.profiles, livreur, adresse })
  }
      ...
      <div className="admin-split-grid" style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 400px' : '1fr', gap: 20 }}>
        ...
        {selected && (
          <div className="card" style={{ position: 'sticky', top: 20, alignSelf: 'start' }}>
            ...
            {selected.adresse && (
              <div style={{ marginBottom: 12, padding: '10px 12px', background: 'var(--info-dim)', borderRadius: 8, border: '1px solid var(--info)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: 11, textTransform: 'uppercase', color: 'var(--info)', fontWeight: 700, marginBottom: 4 }}>
                      📍 Adresse de livraison
                    </div>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>{selected.adresse.label}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{selected.adresse.adresse}</div>
                    {selected.adresse.ville && <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{selected.adresse.ville}</div>}
                  </div>
                  <button className="btn btn-ghost btn-sm" onClick={() => partagerAdresse(selected.adresse)} title="Partager l'adresse">
                    🔗 Partager
                  </button>
                </div>
                {selected.adresse.latitude && (
                  <a href={`https://maps.google.com/?q=${selected.adresse.latitude},${selected.adresse.longitude}`}
                    target="_blank" rel="noreferrer"
                    style={{ fontSize: 11, color: 'var(--info)', marginTop: 6, display: 'inline-block' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div className="flex items-center justify-between" style={{ padding: '8px 12px', background: 'var(--bg-elevated)', borderRadius: 8 }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={async () => {
                  const email = selected.cmd.profiles?.email
                  const nom = selected.cmd.profiles?.nom
                  if (!email) { toast.error('Email client introuvable'); return }
                  await emailEnvoiDocument(email, nom, 'BC', selected.cmd.numero_commande, selected.cmd.numero_commande)
                  toast.success('BC envoyé par email !')
                }}>
                  ✉ Envoyer
                </button>
              </div>
              ...
              {selected.bl ? (
                <div className="flex items-center justify-between" style={{ padding: '8px 12px', background: 'var(--info-dim)', borderRadius: 8 }}>
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={async () => {
                    ...
                    const nom = selected.cmd.profiles?.nom
                    if (!email) { toast.error('Email client introuvable'); return }
                    await emailEnvoiDocument(email, nom, 'BL', selected.bl.numero_bl, selected.cmd.numero_commande)
                    toast.success('BL envoyé par email !')
                  }}>
                    ✉ Envoyer
                  </button>
                </div>
